...
Grid.mixin({
	...
	getEventTimeText: function(range, formatStr, displayEnd) {
		...
		if (formatStr == null) {
			...
			formatEnd = this.eventTimeFormat;
	...
	//i added this for display the end date
		getEventTimeTextForEnd: function(range, formatEnd, displayEnd) {
		...
		if (this.displayEventTime && range.start.hasTime()) {
			if (displayEnd && range.end) {
				return this.view.formatRange(range, formatEnd);
			}
			else {
				return range.end.format(formatEnd);
			}
		}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			timeTextend = this.getEventTimeTextForEnd(event);
			...
			if (timeText) {
				...
				if(timeTextend.length==2)
					{
						
						timeTextend =timeTextend+":00"
					}
				timeHtml = '<div class="fc-time">' + "  מ : "+timeText+"  "+" עד: "+ timeTextend+"  "  +'<span class="boldusername">'  +event.username+ '</span>'+    " :ע'י" + '</div>';
			}
		}

		titleHtml =
			'<div class="headlinerapper">'+
				'<span class="'+event.username+' ">' +
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content" username="'+event.username+'">' +
					(this.isRTL ?
					timeHtml + ' ' + titleHtml:	titleHtml + ' ' + timeHtml 
						) +
